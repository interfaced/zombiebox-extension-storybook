...
zb.storybook.ui.Scene = class extends zb.layers.CuteScene {
	...
	constructor() {
		...
		/**
		 * @type {function(zb.storybook.Story, Object<symbol, zb.storybook.Hook>)}
		 * @protected
		 */
	...
	addStoriesSet(title, storiesSet) {
		...
		newMenuItem.on(newMenuItem.EVENT_OPTION_SELECTED, (eventName, storyName) => {
			...
			if (typeof story !== 'object') {
				throw new Error(
					`Story "${storyName}" from set "${title}" has a wrong type: ` +
			...
			if (this._selectedStory) {
				this._selectStoryDebounced(story, hooks);
			} else {
	...
	_selectStory(story, hooks) {
		...
		if (this._selectedStory) {
			teardownPromise = Promise.resolve(
				hooks[zb.storybook.BEFORE_TEARDOWN] && hooks[zb.storybook.BEFORE_TEARDOWN]()
			);
			...
			teardownPromise = teardownPromise.then(() => Promise.resolve(
				this._selectedStory.teardown && this._selectedStory.teardown())
			);
			...
			teardownPromise = teardownPromise.then(() => Promise.resolve(
				hooks[zb.storybook.AFTER_TEARDOWN] && hooks[zb.storybook.AFTER_TEARDOWN]())
			);
		...
		teardownPromise.then(() => {
			let setupPromise = Promise.resolve(
				hooks[zb.storybook.BEFORE_SETUP] && hooks[zb.storybook.BEFORE_SETUP]()
			);
			...
			setupPromise = setupPromise.then(() => Promise.resolve(
				hooks[zb.storybook.AFTER_SETUP] && hooks[zb.storybook.AFTER_SETUP]())
			);
